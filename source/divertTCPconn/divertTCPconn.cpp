...
int __cdecl main(int argc, char **argv)
{
	...
	default:
		fprintf(stderr, "Usage: %s original_dstport new_dstport [192.168.30.160] [debug]\n",
			argv[0]);
		fprintf(stderr, "Examples:\n");
		fprintf(stderr, "\t%s 3389 31337 \n", argv[0]);
		fprintf(stderr, "\t%s 3389 31337 192.168.30.160 \n",
			argv[0]);
		fprintf(stderr, "\t%s 3389 31337 192.168.30.160 debug\n",
			argv[0]);
		exit(EXIT_FAILURE);
	}
	...
	if (source_address != nullptr) {
		cx = sprintf_s(filter, sizeof(filter),
			"((inbound and tcp.DstPort == %s and ip.SrcAddr == %s)"
			" or (outbound and tcp.SrcPort == %s ))",
			argv[1], source_address, argv[2]);
	} else {
		cx = sprintf_s(filter, sizeof(filter),
